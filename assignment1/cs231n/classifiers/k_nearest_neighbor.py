...
class KNearestNeighbor(object):
    ...
    def compute_distances_no_loops(self, X):
        ...
        # formulate: (a-b)^2 = a^2 + b^2 - 2ab
        X2 = (X**2).sum(axis=1, keepdims=True)
        X_train2 = (self.X_train**2).sum(axis=1)
        X_X_train = X.dot(self.X_train.T)
        dists = np.sqrt(X2 + X_train2 - 2*X_X_train)
        #######################################################################
        #                         END OF YOUR CODE                              #
        #######################################################################
        return dists
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in xrange(num_test):
            ...
            ###################################################################
            neighbors_idx = np.argsort(dists[i])
            closest_y = self.y_train[neighbors_idx[:k]]
            ###################################################################
            # TODO:                                                                 #
